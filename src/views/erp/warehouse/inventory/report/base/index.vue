		...
		<el-input  v-model="queryParams.search" @input="handleQuery" clearable placeholder="请输入SKU" style="width: 250px;" class="input-with-select" >
		 ...
		 <div class="con-body">
			 <GlobalTable ref="globalTable"
			  ...
			  :tableData="tableData"  height="calc(100vh - 180px)" @selectionChange='handleSelect' 
			  ...
			  style="width: 100%;margin-bottom:16px;">
				...
				<el-table-column label="仓库" prop="warehouse"    sortable="custom" >
			 	...
			 	<el-table-column label="待出库" prop="outbound"  width="150" sortable="custom" >
					<template #default="scope">
						<el-popover
						  v-if="scope.row.outbound>0"
						  placement="top"
						  trigger="hover"
						  @show="findOutboundDetail(scope.row)"
						>
						  <p v-for="(item,index) in outboundlist" style="margin-top: 3px;"><span>{{item.name}}:</span>
						  <el-tag class="ml-2" type="warning" >{{item.quantity}}</el-tag></p>
						  <template #reference>
								<span>
									{{scope.row.outbound}}
								</span>
