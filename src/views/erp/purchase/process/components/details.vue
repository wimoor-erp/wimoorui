...
<script setup>
	...
	saveRecord,stopAssemblyEvent,assemblyCancelInstock,resetAssForm,assemblyCompareToSku } from '@/api/erp/assembly/assemblyApi.js';
		...
		getData({"formid":id}).then((res)=>{
			if(res.data.asFormEntryList){
				...
				state.tableData.forEach(item=>{
					if(!item.subnumber){
						item.subnumber=parseInt(res.data.warehouseform.amount)/parseInt(item.amount);
					}
				})
