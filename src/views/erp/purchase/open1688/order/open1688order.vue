 ...
 <div>
	 <el-form label-width="120px">
	 	<el-form-item label="订单地址"  >
			 <el-row v-if="addressList.length" :gutter="20">
			           <el-col :span="24">
						   <el-radio-group v-model="formData.addressid" @change="addressChange" >
							 <el-radio v-for="item in addressList"   :label="item.id">{{item.fullName}}</el-radio>
						   </el-radio-group>
			           </el-col>
			           <el-col :span="24">
	  ...
	  <el-form-item label="产品"   >
		   <el-col v-if="productinfo&&productinfo.specId" :span="24">
		        <div  class="product-box">
					...
					class="img-40"  
					width="40" height="40"  ></el-image>
		        	<div>
		        		<p class="name">{{productinfo.name}}</p>
		        		<p class="sku">{{productinfo.skuInfos}}</p>
		        	</div>
					<div>
						<p class="name">价格</p>
						<p class="text-orange" style="font-size:18px;">￥{{productinfo.price}}</p>
					</div>
		        </div>
		   </el-col>
	  ...
	  <el-form-item label="订单预览"   >
	  		   <el-col v-if="productinfo&&productinfo.specId" :span="24">
					 ...
					 <el-button style="margin-top:20px;" @click="submitOrder">下单</el-button>
	  		   </el-col>
	  		   <el-col v-else :span="24">
	  			    : 未找到预览信息
	  		   </el-col>
...
<style scoped>
    ...
    .img-40{width: 40px;height: 40px;flex: none;margin-top:10px;margin-right: 8px;}
	.sum-p-t{
		display: flex;
