    ...
    <div class="main-sty">
        ...
        <el-row>
            <el-space >
				...
				<el-select v-model="queryParams.ftype" @change="handleQuery">
					<el-option   value="sku" key="sku" label="按SKU汇总"></el-option>
					<el-option   value="supplier" key="supplier" label="按供应商汇总"></el-option>
				</el-select>
				 <Warehouse    @changeware="wareChange" defaultValue="all" />
				 	<Datepicker longtime="ok" ref="datepickers" @changedate="changedate" />
                <el-input  v-model="queryParams.search" @clear="handleQuery" placeholder="请输入产品SKU" class="input-with-select" clearable>
                    <template #append>
        ...
        <el-row>
			...
			show-summary
			 ...
			 :tableData="tableData"  height="calc(100vh - 170px)" 
...
<script setup>
	...
	import {format,dateFormat,dateTimesFormat,formatFloat} from '@/utils/index';
	 ...
	 const { queryParams,tableData,summary,selecttype } = toRefs(state);
