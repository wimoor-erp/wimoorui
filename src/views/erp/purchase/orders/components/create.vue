<template>
	<div class=" el-white-bg">
					 <el-scrollbar class="he-scr-car" @scroll="scroll">
						 ...
						 <div class="mark-re">
						   <h5 >商品列表({{skunum}}) <span class="text-orange" style="padding-left:10px">￥{{outputmoney(totalprice)}}</span> </h5>
						   <div>
						   	<el-space>
								<el-tooltip
										 effect="dark"
					...
					if(row.issfg=="1" && row.checkdsub==true){
						if(row.subsku && row.subsku.length>0){
							row.subsku.forEach(function(sub){
								...
								state.totalprice=parseFloat(state.totalprice)+parseFloat(sub.totalprice);
								state.skunum++;
								if(type!="refresh"){
						...
						row.totalprice=formatFloat(parseFloat(row.num)*parseFloat(row.price));
						state.totalprice=parseFloat(state.totalprice)+parseFloat(row.totalprice);
						state.skunum++;
					}
				}
			});
