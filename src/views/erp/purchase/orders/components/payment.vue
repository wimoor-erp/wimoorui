		  ...
		  >
		    <el-tab-pane :label="operateType.tabsType" name="1" >
				<el-row :gutter="24">
					...
					<el-card class="m-b-16 " shadow="never" style="margin-bottom:10px;" v-if="operateType.tabsType=='付款'">
...
<script setup>
	...
	function handleClick(val){
		if(val=="2"){
			...
			recordRef.value.show(state.queryParams.entry.id,ftypes);
		}
	}
	function handleAdd(){
	...
	function loadDetail(){
		purchaselistApi.getRecdetail({"id":state.queryParams.entry.id,"ftype":"pay","paytype":"out"}).then((res)=>{
			if(res.data){
	...
	function applypayment(){
		...
		data.shipamount=state.formData.ship;
		data.costamount=state.formData.cost;
		...
		if(data.paytype=="in"){
			if(state.formData.ship>0){
				data.shipamount=(-1)*state.formData.ship;
			}
			if(state.formData.cost>0){
				data.costamount=(-1)*state.formData.cost;
			}
		}
