	...
	<el-row :gutter="24" v-if="state.LogisticsResult.length>0">
		<el-col  :span="10">
			...
			<el-descriptions title="物流单" size="small" :column="1">
			 ...
			 <div class="flex-center product-list" v-for="product in orderResult.productItems">
				 <el-image  :src="product.productImgUrl[0]"></el-image>
			     <div class="font-small">
					 <p class="name">{{product.name}}</p>
					 <el-space class="m-t-8" :size="24">
						 <span><span class="font-extraSmall">单位重量: </span> -</span>
						 <span><span class="font-extraSmall">数量: </span>  {{product.quantity}}{{product.unit}}</span>
					 </el-space>
				 </div>
			 </div>
			 </el-card>
		...
		<el-card class="gray-bg" shadow="never" v-for="trace in logisticsTrace">
		    <template #header>
				...
				<el-space>
					...
					<div class="font-extraSmall">物流编号:{{trace.logisticsId}}</div>
				</el-space>
		    </template>
		    <el-timeline>
		       <el-timeline-item
