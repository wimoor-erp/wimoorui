<template>
	...
	<el-table  :data="skuList" :expand-row-keys="expandRowKeys" height="calc(100vh - 150px)"  :row-class-name="rowClassName" row-key="id">
				...
				<div class="subAssemblyShelfTable" v-if="props.row.assemblyList&&props.row.assemblyList.length>0">
			     <el-table :data="props.row.assemblyList" :row-class-name="rowClassName"  row-key="id">
					...
					<el-table-column label="需求量"  width="120px" prop="subamount">
						<template #default="scope">
							 <div style="padding-left: 5px;" :class="scope.row.subamount!=scope.row.sumout&&(!scope.row.shelfInvRecordList||scope.row.shelfInvRecordList.length==0)?'text-red':'text-black'"> {{scope.row.subamount}}</div>
						   <div  >
							   <el-popover
							      placement="top-start"
							      title="下架记录"
		...
		<el-table-column label="发货"   width="120px" prop="amount">
				<template #default="scope">
					<div style="padding-left: 5px;" :class="scope.row.amount!=scope.row.sumout&&(!scope.row.assemblyList||scope.row.assemblyList.length==0)&&(!scope.row.shelfInvRecordList||scope.row.shelfInvRecordList.length==0)?'text-red':'text-black'"> {{scope.row.amount}}</div>
				    <div>
					   <el-popover
					      title="下架记录"
					      :width="400"
		...
		if(state.skuList&&state.skuList.length>0){
			state.skuList.forEach(row=>{
				var hasqty=0;
				 ...
				 var out=getOut(row);
				 if(out+hasqty>parseInt(row.amount)){
					 needwarning=true;
				 }
