			...
			<el-card  >
				 <template #header>
				      <div class="card-header">
				        <span>辅料列表</span>
				        <span class="font-extraSmall">- {{shipdata.warehouse}}</span>
				      </div>
				</template>
			<el-table :data="materiallist.list" border >
				...
				<el-table-column prop="needamount" label="扣减数量" width="100"  >
					<template  #default="scope">
						...
						<el-input v-else type="number"  v-model="scope.row.needamount" @input="scope.row.needamount=CheckInputInt(scope.row.needamount)" ></el-input>
					</template>
				</el-table-column>
			...
			<el-table :data="boxListData.list" border >
				...
				<el-table-column prop="number" label="出库数量"   >
					<template  #default="scope">
						...
						<el-input v-else type="number"  v-model="scope.row.editnumber" @input="scope.row.editnumber=CheckInputInt(scope.row.editnumber)" ></el-input>
					</template>
				</el-table-column>
			</el-table>
			 	</el-card>
...
<script setup>
	...
	import {formatFloat,format,dateFormat,dateTimesFormat,CheckInputInt,CheckInputFloat} from '@/utils/index';
	...
	const emit = defineEmits(['stepdata',"change"]);
		...
		var param={sumBoxWeight:0,sumBoxVolume:0};
			...
			function showFeeDialog(){
				//查询itemlist 然后show
				shipdata.value.itemList.forEach(item=>{
				...
				editFeeRef.value.show(shipdata.value.itemList);
			}	
