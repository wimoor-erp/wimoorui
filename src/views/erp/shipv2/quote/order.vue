										 	...
										 	<div class="flex-center-between font-base">
										 			<el-space :size="24">
														...
														<div v-if="scope.row.closetime">
															...
															<span>{{dateFormat(scope.row.closetime)}} </span>
														</div>
										 				<p>
														...
														<tr class="adress-tr" v-else>
										 				...
										 				<tr v-for="sh in sub.shipmentList" :key="sh.buyerid" >
															...
															<div class="font-extraSmall">{{sh.number}}</div>
										 					...
										 					<td>{{sh.weight}}</td>
										 					<td>{{sh.volume}}
																<span class="font-bold">{{formatFloat(sh.volume/1000000.0)}} m³</span>
															</td>
										 					<td>
										 						<el-popover
										 						 trigger="hover"
										 						...
										 						<el-table :data="sh.quotationPriceList">
										 							...
										 							<el-table-column label="计价方式" width="140">
										 								<template #default="scope">
										 									<p>
										 										<span>材积重：</span>
										 							...
										 							<el-table-column label="其他费用(￥)" width="100"  align="right" prop="otherfee" >
										 								<template #default="scope">
										 									{{scope.row.otherfee?.toFixed(2)||0.00}}
										 								</template>
										 							</el-table-column>
																	<el-table-column label="附加费用(￥)" width="100" align="right" prop="otherfee" >
										 								<template #default="scope">
										 									{{scope.row.tax?.toFixed(2)||0.00}}
										 								</template>
										 							</el-table-column>
										 		...
										 		<div class="cus-list" v-for="sub in scope.row.quotationPriceList" :key="sub.id">
										 			<div class="flex-row-top">
										 				<el-space class="cus-name">
										 					<el-icon :class="sub.confirm?'text-green':''" class="user"><UserFilled /></el-icon>
																<el-space v-if="isowner">
																<el-tooltip  style="float:left;"   :content="sub.supplier.name"><div  class="text-omit-2">{{sub.supplier.name}}</div></el-tooltip>
																<copy style="float:right;"  @click.stop="CopyText(urlFormat(sub.supplier))" title='复制SKU' theme="outline" size="14" fill="#666"/>
															    </el-space>
										 					<div v-else>{{sub.supplier.name}}</div>
										 				</el-space>
										 				<div class="flex-center">
										 					<el-space direction="vertical" :size="0">
										 						<p style="width:110px"><span class="font-extraSmall">附加费用:
										 						<span v-if="sub.tax">{{sub.tax.toFixed(2)||0.00}}</span>
																<span v-else>0.00</span>
										 						</span>
										 						</p>
										 					 
																<p style="width:110px"><span class="font-extraSmall">总价:</span>
...
<script setup>
	import {Search,ArrowDown,Edit,Delete,UserFilled,ChatLineSquare,Link} from '@element-plus/icons-vue'
	...
	const{
		 selectRows,quoteVisiable,quoteOrder,orderTableData,queryOrderParam,selectOrderRows,shipmentVisiable,
