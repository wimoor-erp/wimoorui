<template>
	<div class="box-margin">
		<div class="pag-radius-bor mar-bot" >
			<div class="con-body lylStepTwo" > 
			  ...
			  <div class="font-base m-b-8">货件数 {{shipmentNum}}</div>
			  ...
			  <el-row :gutter="16">
				  <el-col :span="8" v-for="(item,index) in tableList" :key="index" >
					  ...
					  <el-card shadow="none">
						  <template #header>
                              <div class="font-base font-bold">货件 {{index+1}}</div>
						  </template>
						  <el-space 
						  class="shipment-message-header"
						  direction="vertical" alignment="normal">
								  <div class="font-base">
									  <span class="light-font"style="width:85px" >	货件名称： </span>
									  <span>{{item.shipment.name}}</span>
								  </div>
								  <div class="font-base">
									  <span class="light-font" style="width:85px" >	货件编码： </span>
									  <span>{{item.shipment.shipmentConfirmationId}}</span>
								  </div>
								  <div class="font-base flex-center ">
									  <span class="light-font " style="width:85px">	发货地址： </span>
								  ...
								  <div class="font-base flex-center ">
									  <span class="light-font " style="width:85px"  >	收货地址： </span>
									  <span class="text-omit-1">
						  ...
						  <div class="shiment-goods-wrap">
							  <div class="s-g-header">
								  <el-space size="large">
									  <div>
										  <span>SKU数量：{{item.skunum}}</span>
									  </div>
									  <div>
										  <span>商品总数：{{item.sumqty}}</span>
									  </div>
									  <div>
										  <span>重量：{{item.totalweight}}kg</span>
									  </div>
									  <div>
										  <span>尺寸：{{item.totalBoxSize}}</span>
									  </div>
								  </el-space>
							  </div>
							  <div class="s-g-body">
							  ...
							  <div class="s-g-footer">
								  <el-row>
									  <el-col :span="24">
										 ...
										 <div class="flex-center-between font-base">
										 	<span >配送方式：<el-button link v-if="transportationloading" :loading="true"></el-button></span>
											<div class="flex-center-between"> 
											      <el-select :loading="transportationloading" 
												   ...
												   v-model="item.shipment.transportationOptionId" >
															 <el-option 
																	 v-for="itemtrans in transportation[item.shipment.shipmentid]" 
					   ...
					   <el-select v-model="item.shipment.printType">
							<el-option v-if="item.plan.transtyle==='SP'" v-for="value,key,index in item.pkgpaper" :value="key" :label="value"></el-option>	
							<el-option v-else v-for="value,key,index in item.pkgpaperltl" :value="key" :label="value"></el-option>	
					   </el-select>
						 <el-dropdown>
	...
	<Table ref="tableDialogRef" ></Table>
	 ...
	 <div style="width:100%;overflow-x: auto;margin-bottom:10px;">
	   <ShipBox ref="shipBoxRef" @change="handleBoxSaveChange"></ShipBox>
	 </div>
	 </div>
...
<script setup>
	...
	function loadTransportation(withOutGenerate,paginationToken){
		...
		"paginationToken":paginationToken}).then((res)=>{
			if(res&&res.data&&res.data.pagination&&res.data.pagination.nextToken){
				...
				loadTransportation(withOutGenerate,res.data.pagination.nextToken);
			}else{
