					   ...
					   <el-row >
						   ...
						   <el-col :span="6"  >
							  <div class="text-right">
								  <el-space>
									   <el-button   v-if="planData.auditstatus>=5" style="width:120px" @click="getShipments">重新同步货件</el-button>
...
<script setup>
	...
	let{planData,tableData,submitconfirmloading,transportation,paper,shipall,deliveryWindowOptions,
	    options,placementOptionId,shipments,shipmentloading,optionsloading}=toRefs(state);
		...
		if(state.option){
			...
			state.option.shipmentIds.forEach(async shipmentid=>{
				...
				await shipmentPlacementApi.getShipment({"planid":planid,"shipmentId":shipmentid}).then(async res=>{
					...
					await  shipmentPlacementApi.listShipmentBoxes({"formid":planid,"shipmentid":shipmentid}).then(res=>{
								shipment.boxinfo=res.data;	   
					});
