					...
					<el-row>
						...
						<el-col :span="12">
							...
							<!-- <el-form-item label="预计发货日期">
									<el-date-picker
										  v-model="formData.preShippingDate"
										  type="date"
										  placeholder="选择日期"
										/>
							</el-form-item> -->
...
function loadInplace(){
 ...
 watch(() =>router.currentRoute.value.query,(newValue,oldValue)=> {
           ...
           function manageAlladdr(){
			 ...
			 function getPreShippingDate(transtype){
				 var day=1;
				 if(transtype){
					 var trans=null;
					 state.transTypeAllList.forEach(item=>{
						 if(item.id==transtype){
							 trans=item;
						 }
					 });
					 if(trans&&trans.day){
 			 ...
 			 function submitPlan(){
				 ...
				 state.formData.preShippingDate=getPreShippingDate(state.formData.transtype);
