				  ...
				  >
				   ...
				   <el-form 
					 ...
					 label-position="top">
					   <el-form-item label="付款金额"  prop="price">
						   <el-input v-model.number="from.price" >
				   ...
				   <!-- <el-popover :visible="scope.row.returnVisible" placement="left"   trigger="click" >
				      ...
				      <div class="m-t-8">
				        ...
				        <el-button size="small" type="primary" @click="scope.row.returnVisible = false"
				          >作废</el-button
				        >
				      </div>
				      <template #reference>
						  <el-button type="primary" link @click="scope.row.returnVisible=true">作废</el-button>
				      </template>
				    </el-popover> -->
			</template>
		</el-table-column>
...
<script setup>
	import {h,ref,reactive,toRefs,defineEmits,} from 'vue';
	...
	function handleShow(row){
		state.from.amount = row.amount-row.paid
	}
	/* 确认付款 */
