<template >
	...
	:defaultSort="defaultSort" 
	 ...
	 <template #field>
	  <el-table-column label="图片"  fixed width="65" prop="image" >
	  	<template #default="scope">
	  		<el-image class="product-image" :src="scope.row.image"></el-image>
	  	</template>
	  </el-table-column>
	  <el-table-column v-if="activeName=='1'" label="ASIN" fixed width="100" prop="asin" />
	  <el-table-column v-if="activeName=='3'" label="名称/SKU" fixed  width="200" show-overflow-tooltip>
	  		 <template #default="scope">
	  		 <span class="name">{{scope.row.cname}}</span>
	  		 <p class="sku">{{scope.row.sku}}</p>
	  		 </template>
	  </el-table-column>
		<el-table-column v-else label="名称/MSKU" fixed  width="200" show-overflow-tooltip>
				 <template #default="scope">
				 <span class="name">{{scope.row.name}}</span>
				 <p class="sku">{{scope.row.msku}}</p>
				 </template>
		</el-table-column>
		<el-table-column label="店铺/国家" fixed width="100"  >
			<template #default="scope">
				<p>{{scope.row.group}}</p>
