<template>
	<div   class="main-sty">
		  <el-row>
			  	<div class="con-header" style="margin-bottom:20px;">
					<el-space >
						<el-select v-model="selectable" style="width:150px" @change="handleQuery">
							 <el-option value="sku" key="sku" label="SKU头程"></el-option>
							 <el-option value="detail" key="detail"  label="SKU头程明细"> </el-option>
						</el-select>
				 ...
				 <el-input  v-model="queryParam.search" clearable @input="handleQuery" placeholder="请输入SKU或货件编码" class="input-with-select" >
					<template #append>
	...
	<GlobalTable
	  ref="globalTable" :tableData="tableData"   height="calc(100vh - 170px)" @loadTable="loadTableData" :stripe="true"  style="width: 100%;margin-bottom:16px;">
		 <template #field>
		 <el-table-column label="产品信息" prop="msku" v-if="selectable!='history'" sortable="custom"  show-overflow-tooltip   >
		 ...
		 <el-table-column prop="ownername"   width="300"    v-if="selectable!='history'" sortable="custom"  label="产品负责人"   >
			  <template #default="scope">
		 ...
		 <el-table-column prop="warehouse"    v-if="selectable=='detail'" sortable="custom"   label="发货信息"  >
			 <template #default="scope">
				 <div>{{scope.row.warehouse}}</div>
				   <div><span class="font-extraSmall">状态:</span>{{scope.row.statusname}}</div>
			  </template>
		 </el-table-column>
		 <el-table-column prop="ShipmentId"    v-if="selectable=='detail'"   sortable="custom" label="货件信息"   >
			 <template #default="scope">
		 ...
		 <el-table-column prop="qty"   sortable="custom"   width="100"  label="发货数量"   />
		 <el-table-column prop="skufee"   width="150"   v-if="selectable=='sku'"  sortable="custom"  label="SKU头程费用(元)"   />
		 <el-table-column prop="avgfee"   width="200"   v-if="selectable=='sku'"  sortable="custom"  label="平均单件头程费用(元)"   />
		 <el-table-column prop="transweight"    v-if="selectable=='detail'" sortable="custom"   label="实际结算重量"   >
				 <template #default="scope">
					  <div>{{scope.row.transweight}}(kg)</div>
					   <div><span class="font-extraSmall">SKU材积:</span>{{scope.row.skuweight}}</div>
				 </template>
		 </el-table-column>
		  <el-table-column prop="shipmentfee"  width="120"    v-if="selectable=='detail'"  sortable="custom"  label="货件运费(元)"   />
		 <el-table-column prop="skufee"   width="130"  v-if="selectable=='detail'"   sortable="custom" label="SKU费用分摊"   >
					 <template #default="scope">
						  <div>￥{{formatFloat(scope.row.skufee)}}</div>
						   <div><span class="font-extraSmall">￥{{formatFloat(scope.row.skufeeavg)}}/个</span></div>
					 </template>
		 </el-table-column>
		  <el-table-column prop="msku"    v-if="selectable=='sku'"  width="100"   label="操作"   >
			  <template #default="scope">
