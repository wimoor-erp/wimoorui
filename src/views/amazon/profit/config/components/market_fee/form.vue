<template>
	...
	<el-form-item label="是否计算库存配置费"  v-if="country=='US'">
	     <template #label>
			 <div class="text-right">
...
<script setup>
	...
	onMounted(()=>{
		configApi.findInplacefee({"country":props.country.toLowerCase()}).then(res=>{
			 state.inplacefeeOptions=res.data;
		});
	})
	let {marketData,fbaMonthsOptions,manualProcessingFeeOptions,countryMapData,shipmentstyle,
	     storageFee,country,currency,currencyicon} =toRefs(props);
	let {inplacefeeOptions} =toRefs(state);
