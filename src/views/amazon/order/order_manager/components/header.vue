...
<script>
	...
	export default{
		...
		emits:["refreshTable","downloadOrderlist"],
		setup(props,context){
			...
			function refreshTable(){
				...
				if(rows.search!="" && rows.search!=undefined){
					if(distinctDay(rows.dates[0],rows.dates[1])<=365){
						context.emit("refreshTable",rows)
					}else{
						//提示
			...
			function handleCallbackReview(){
					context.emit("callbackReview",rows);
			}
			//获取订单状态
			...
			function downloadOrderlist(){
				context.emit("downloadOrderlist",rows)
			}
			//日期改变
			function dateChange(){
				refreshTable()
			...
			return{
				getstoreData,getCountryData,store,country,changeGroup,orderTypeList,refreshTable,customerTypeList,customerType,downloadOrderlist,
				dateValue,shortcuts,dateChange,storeList,countryList,orderType,searchKeywords,sumdataShow,submitForm,
				searchConfirm,searchTypeChange,resetForm,moreSearchVisible,color,colorList,colorChange,spacer,sumSale,selectlabel,shipTypeList,shipType,
			    invoiceManage,dataType,handleCallbackReview,
			}
		}
	}
</script>
