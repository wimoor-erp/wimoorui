<template>
	...
	<GlobalTable  ref="globalTable" :tableData="tableData"   height="calc(100vh - 246px)" @loadTable="loadTableData" :stripe="true"  style="width: 100%;margin-bottom:16px;">
	    ...
	    <el-table-column prop="purchaseDate" label="订单时间" width="120"   >
			<template #default="scope">
			    {{dateFormat(scope.row.purchaseDate)}} 
			</template>
		</el-table-column>	
		<el-table-column prop="lastUpdatedDate" label="更新时间" width="120"   >
			<template #default="scope">
			    {{dateFormat(scope.row.lastUpdatedDate)}} 
			</template>
		</el-table-column>
	    <el-table-column prop="requestedQuantity" label="请求数量" width="100"    />
		<el-table-column prop="inProcessQuantity" label="处理数量"  width="100" />
