<template>
		<el-tag class="pointer" v-if="feed_processing_status=='_DONE_' || feed_processing_status=='DONE'" title="请求已处理" @click="downloadFeedFile" type="success"  :closable="false"/>
		<el-tag class="pointer" v-else-if="feed_processing_status" :title="feed_processing_name" @click="downloadFeedFile" type="warning"  :closable="false"/>
		<el-tag class="pointer" v-else title="未提交" type="info"  :closable="false"/>
</template>
...
<script>
	...
	export default {
		...
		setup(prop,context){
			...
			return {submitfeedInfo ,feed_processing_status,feed_processing_name,downloadFeedFile}
			}
	}
